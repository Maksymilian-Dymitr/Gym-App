...
export async function signin(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const result = await AdminAuthService.createUser(email, password, "User");
...
export async function googleCallback(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (!code) return reply.status(400).send({ error: "Authorization code not provided" });
    ...
    return reply.redirect(
      `http://localhost:3000/login-success?token=${accessToken}&userId=${user.id}&role=${user.role}&email=${user.email}`
    );
  } catch (error: any) {
...
export async function refreshToken(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    // Verify the cookie token directly (not the Authorization header)
    const decoded = request.server.jwt.verify<TokenPayLoad>(refresh_token);
    if (decoded.type !== "refresh") {
      return reply.status(401).send({ message: "Not Authorized" });
    }

    const accessToken = await reply.jwtSign(
      { id: decoded.id, email: decoded.email, role: decoded.role, type: "access" },
      { expiresIn: "1h" },
    );
